...
export function setupSky(scene) {
    ...
    sunMesh = new THREE.Mesh(new THREE.SphereGeometry(5, 32, 32), new THREE.MeshBasicMaterial({ color: 0xFFFF00 }));
    scene.add(sunMesh);

    moonMesh = new THREE.Mesh(new THREE.SphereGeometry(3, 16, 16), new THREE.MeshBasicMaterial({ color: 0xDDDDFF }));
    scene.add(moonMesh);
...
export function createClouds() {
    ...
    for (let i = 0; i < 30; i++) {
        ...
        const mat = new THREE.MeshStandardMaterial({ 
            color: isRain ? 0x555555 : 0xffffff, transparent: true, opacity: 0.9, flatShading: true 
        });
...
export function updateSky(scene, cycleTime) {
    ...
    if (sunHeight > 0.2) { 
        targetColor = CONFIG.colors.skyDay; fogColor = 0x87CEEB; lightIntensity = 1.5; 
    } else if (sunHeight > -0.2) { 
        targetColor = CONFIG.colors.skySunset; fogColor = 0xFF4500; lightIntensity = 0.5; 
    } else { 
        targetColor = CONFIG.colors.skyNight; fogColor = 0x050510; lightIntensity = 0.0; 
    }
