...
// Material da Estrada (Terra batida)
// Aumentei o tamanho e a rugosidade
const roadGeo = new THREE.CircleGeometry(0.7, 8); 
roadGeo.rotateX(-Math.PI / 2);
const roadMat = new THREE.MeshStandardMaterial({ color: 0x654321, flatShading: true }); // Marrom mais escuro
...
function createRoads(kingdoms, seeds) {
    // Remove estradas antigas se existirem
    if (roadsMesh) { 
    ...
    kingdoms.forEach(k => {
        k.villages.forEach(v => {
            ...
            // Mais densidade de pontos (0.6 metros entre cada ponto)
    ...
    pathPoints.forEach((p, i) => {
        ...
        // Se a estrada cair na água, levanta ela pra virar uma "ponte" ou ignora
        if (h < 0.3) h = 0.3;

        // --- O SEGREDO ESTÁ AQUI ---
        // Adicionei +0.15 na altura. Isso garante que a terra fique ACIMA da grama.
    ...
    console.log(`Estradas geradas: ${pathPoints.length} segmentos.`);
}

// --- MODELOS (IGUAL AO ANTERIOR) ---
